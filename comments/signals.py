...
def notice_and_achieve_on_comment(instance, created=False, *args, **kwargs):
    ...
    if hasattr(target, 'author_id') and created and target.author_id != instance.author_id:
        Notification.objects.create(
            ...
            content='{0} прокомментировал ваш {1} с id = {2}'.format(instance.author, target._meta.verbose_name,
                                                                     target.id)
        )
        ...
        if num in comments.keys():
            Achievement.objects.update_or_create(
                ...
                title=comments[num],
                defaults={
                    'content': 'У вас {0} комментов на {1} с id = {2}!'.format(num, str(target._meta.verbose_name),
                                                                               target.id)
                }
            )
